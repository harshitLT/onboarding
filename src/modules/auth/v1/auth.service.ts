...
export class AuthService {
  ...
  public async login(user: UserDocument): Promise<object> {
    try {
      if (await this.tokenModel.findOne({ userId: user._id })) {
        throw new HttpException(
          'Please logout of existing application to continue',
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  private async generateAuthTokens(user: UserDocument): Promise<object> {
    ...
    const accessTokenExpires =
      this.configService.get('JWT_ACCESS_EXPIRATION_MINUTES') * 60;
    const accessToken = this.jwtService.sign(
      { userId: user._id, type: TokenType.ACCESS },
      {
    ...
    const refreshTokenExpires =
      this.configService.get('JWT_REFRESH_EXPIRATION_DAYS') * 24 * 60 * 60;
    const refreshToken = this.jwtService.sign(
      { userId: user._id, type: TokenType.REFRESH },
      {
